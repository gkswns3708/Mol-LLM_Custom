...
# (MoleculeNetDatasetDeepChem, MolInstructionDatset, SMolInstructDataset 등은 기존과 동일하므로 생략하지 않고 포함)
...
class ChEBIDataset(Dataset):
    ...
    def set_necesary_data(self):
        ...
        for i in tqdm(range(total_len), desc=self.task):
            try:
                graph, label, input_mol_string, instruction = self.get_necessary_data(i)
                processed_label_list.append(label)
                processed_input_mol_string_list.append(input_mol_string)
    ...
    def get_necessary_data(self, index):
        ...
        if self.is_selfies:
            ...
            smiles = mol_data
            try:
                # [중요] 표준 데이터의 SMILES를 SELFIES로 변환
                selfies = sf.encoder(smiles)
                if selfies is None: selfies = smiles 
            except:
                selfies = smiles

        if smiles is None: raise ValueError(f"SMILES decode failed")
        ...
        if self.task in TEXT2MOL_BENCHMARKS:
            ...
            graph = [smiles2data("CC"), smiles2data("CC")] # Dummy Graph
            input_mol_string = "<None>"
...
# =============================================================================
# [get_dataset 수정: liupf/ChEBI-20-MM 사용]
# =============================================================================
def get_dataset(task_name, raw_data_root):
    if "chebi-20" in task_name:
        ...
        try:
            # liupf/ChEBI-20-MM: Standard Split (Train:26k, Val:3.3k, Test:3.3k)
...
if __name__ == "__main__":
    ...
    if trainsets:
        ...
        print(f"Saved Final Train Dataset: {save_name_train} (Size: {len(mapped_trainset)})")
    else:
    ...
    if testsets:
        ...
        print(f"Saved Final Test Dataset: {save_name_test} (Size: {len(mapped_testset)})")
    else:
        print("Warning: No test datasets were concatenated.")
        
    if valsets:
        concat_valset = datasets.concatenate_datasets(valsets)
        print(f"Mapping VALIDATION dataset ({len(concat_valset)} examples)...")
        
        mapped_valset = concat_valset.map(
            prepare_data_instance,
            fn_kwargs={
                "system_prompt": system_prompt,
                "llm_model_name": llm_model 
            },
            num_proc=args.test_procs # Test용 코어 수 사용
        )
        save_name_val = f"{raw_data_root}/{processed_file_name}_validation_{cfg.data_tag}"
        mapped_valset.save_to_disk(save_name_val)
        print(f"Saved Final Validation Dataset: {save_name_val} (Size: {len(mapped_valset)})")
    else:
        print("Warning: No validation datasets were concatenated.")
