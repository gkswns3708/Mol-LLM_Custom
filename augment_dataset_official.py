...
def mol2graph(mol):
    ...
    Returns:
        dict: Graph representation with edge_index, edge_feat, node_feat, and num_nodes.
              All arrays are converted to lists for proper serialization.
              Empty edge_attr is stored as empty list [] - loader should convert to (0, 3) shape tensor.
    """
    # Atoms
    # atoms
    atom_features_list = []
    ...
    else:  # mol has no bonds (single atom molecule)
        edge_index = np.empty((2, 0), dtype=np.int64)
        edge_attr = np.empty((0, num_bond_features), dtype=np.int64)

    graph = dict()
    # Convert to lists for proper dataset serialization
    graph["edge_index"] = edge_index.tolist()
    # For empty edge_attr, store as empty list - loader must handle (0, 3) shape conversion
    graph["edge_feat"] = edge_attr.tolist() if edge_attr.size > 0 else []
    graph["node_feat"] = x.tolist()
...
def _ensure_list(data):
    """Convert numpy array to list if needed, preserve empty list for empty arrays."""
...
def _get_fallback_graph():
    """Return a fallback graph (CCC - propane) for error cases."""
    fallback_mol = Chem.MolFromSmiles("CCC")
    return mol2graph(fallback_mol)
...
def map_by_substructure_replacement(
    ...
    """
    Safe version: handles errors gracefully and ensures proper data serialization.
    All edge_attr fields will be stored as lists (empty list [] for molecules without bonds).
    Loader should convert empty lists to (0, 3) shape tensors.
    """
    ...
    for i in range(num_rejected_graphs):
        try:
            ...
            # Use fallback graph on error
            rejected_graph = _get_fallback_graph()
            additional_rejected_graph = _get_fallback_graph()

        # Store as lists, ensuring empty edge_attr is stored as empty list []
        data_point[f"{i}-th_rejected_x"] = _ensure_list(rejected_graph["node_feat"])
        data_point[f"{i}-th_rejected_edge_index"] = _ensure_list(rejected_graph["edge_index"])
        data_point[f"{i}-th_rejected_edge_attr"] = _ensure_list(rejected_graph["edge_feat"])
        data_point[f"{i}-th_additional_rejected_x"] = _ensure_list(
            additional_rejected_graph["node_feat"]
        )
        data_point[f"{i}-th_additional_rejected_edge_index"] = _ensure_list(
            additional_rejected_graph["edge_index"]
        )
        data_point[f"{i}-th_additional_rejected_edge_attr"] = _ensure_list(
            additional_rejected_graph["edge_feat"]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset_path",
        type=str,
        default="GSAI-ML-LLaDA-8B-Instruct_string+graph_q32_test_512_Truncation",
    )
